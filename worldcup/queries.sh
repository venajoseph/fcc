...
echo "$($PSQL "SELECT t1.name FROM games FULL JOIN teams AS t1 ON (games.winner_id = t1.team_id OR games.opponent_id = t1.team_id) WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY t1.name")"
